...
This project is a web application demonstrating how to strengthen multifactor security authentication using the Vonage Identity Insights API and Verify v2 API, integrated with Firebase Hosting, Functions, and Firestore. The application includes a simple bank dashboard and a login form. If the Identity Insights API detects that a phone number was swapped recently, the verification code will not be sent, and additional security measures will be applied. A verification code will be sent via the Verify v2 API to authenticate the user if no recent swap is detected.
...
1. Clone the repository and change directories
   ...
   cd demo-sim-swap_verifyv2-javascript-multifactor_authentication-firebase
   ```
...
3. Move the `.env.example` file to `.env` file in the project root and include the following environment variables:
   ...
   # Number of hours to check SIM Swap events (1-2400)
   PERIOD=240
   ```

4. You have the choice to set this variable:
   ```bash
...
The application uses the Vonage Identity Insights API to check whether a given phone number has been swapped in the last few days. This protects users from attacks that exploit SIM swaps.

### Verify v2 API

The Verify v2 API sends a one-time code to the user's phone number for authentication. This verification code will be sent if the Identity Insights API determines that the number has not been recently swapped.
...
2. The Identity Insights API checks whether the number was swapped recently.
